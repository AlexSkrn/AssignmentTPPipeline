...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X = X.copy()
        # get dummies
        dummies = pd.get_dummies(X[self.variables], drop_first=True)
        for var in self.dummies:
            if var not in dummies.columns:
                dummies[var] = 0  # add missing dummies if any
        dummies = dummies[self.dummies]  # re-order columns
